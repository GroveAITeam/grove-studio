      ...
      <template v-for="(group, groupName) in groupedConversations">
        <div class="mb-2">
          ...
          <div v-for="(conversation, index) in group" :key="conversation.id"
               :class="['group flex items-center gap-1 px-2 py-1.5 rounded-md cursor-pointer transition-colors',
            ...
            <button @click="(event) => handleDelete(props.conversations.indexOf(conversation), event)"
                    class="opacity-0 group-hover:opacity-100 hover:bg-base-300 p-1 rounded-md transition-all">
              <Icon icon="material-symbols:delete-outline" class="text-base text-base-content/70 hover:text-error" />
            </button>
          </div>
        </div>
      </template>
